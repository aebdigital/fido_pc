...
const InvoiceCreationModal = ({ isOpen, onClose, project, categoryId }) => {
  const { t } = useLanguage();
  const { createInvoice, contractors, activeContractorId, clients, calculateProjectTotalPriceWithBreakdown, generalPriceList } = useAppData();
  ...
  const checkRequiredFields = () => {
    ...
    if (currentContractor) {
      ...
      if (!currentContractor.postalCode) missing.push(`${t('Contractor')}: ${t('Postal code')}`);
      if (!currentContractor.country) missing.push(`${t('Contractor')}: ${t('Country')}`);
      if (!currentContractor.businessId) missing.push(`${t('Contractor')}: ${t('Business ID')}`);
      if (!currentContractor.taxId) missing.push(`${t('Contractor')}: ${t('Tax ID')}`);
      if (!currentContractor.bankAccount) missing.push(`${t('Contractor')}: ${t('Bank account number')}`);
    } else {
    ...
    if (currentClient) {
      ...
      if (!currentClient.postalCode) missing.push(`${t('Client')}: ${t('Postal code')}`);
