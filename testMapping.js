...
// Copied from priceListMapping.js for testing
...
function dbColumnsToPriceList(dbRow, defaultPriceList) {
...
// Mock data
// Create realistic mock of default data based on AppDataContext.js
const defaultData = {
    work: Array.from({ length: 35 }, (_, i) => ({ name: `Work ${i}`, price: 10 })),
    material: Array.from({ length: 26 }, (_, i) => ({ name: `Material ${i}`, price: 10, capacity: { value: 10 } })),
    installations: Array.from({ length: 14 }, (_, i) => ({ name: `Inst ${i}`, price: 10 })),
    others: Array.from({ length: 6 }, (_, i) => ({ name: `Others ${i}`, price: 10 }))
};

// Adjust defaults for VAT etc if needed (VAT is index 5 for others)
defaultData.others[5].price = 23;

// Test case
const dbRow = {
    'work_auxiliary_and_finishing_price': 0,
    'material_auxiliary_and_fastening_price': 0,
    'others_vat_price': 0
};

console.log('Testing...');
const result = dbColumnsToPriceList(dbRow, defaultData);

console.log('Work [34] price (Auxiliary and finishing work):', result.work[34].price);
console.log('Material [25] price (Auxiliary and fastening material):', result.material[25].price);
console.log('Others [5] price (VAT):', result.others[5].price);

if (result.work[34].price === 0 && result.material[25].price === 0 && result.others[5].price === 0) {
    console.log('SUCCESS: 0 values are preserved.');
} else {
    console.log('FAILURE: 0 values are NOT preserved.');
}
